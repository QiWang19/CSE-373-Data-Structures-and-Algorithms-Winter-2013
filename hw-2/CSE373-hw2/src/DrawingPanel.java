 ...
 * @version 2013/01/19
 */
public final class DrawingPanel extends FileFilter implements ActionListener, MouseMotionListener,
	...
	public static final String VERSION = "2013/01/19";
	private static final String TITLE = "Drawing Panel";
	private static final String COURSE_WEB_SITE = "http://www.cs.washington.edu/education/courses/cse373/13wi/drawingpanel.txt";
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() instanceof Timer) {
			...
			JOptionPane.showMessageDialog(frame, "DrawingPanel\n"
	...
	// sets whether the panel will always cover other windows
	...
	private class DiffImage extends JPanel implements ActionListener, ChangeListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (source == box) {
				...
				Color color = JColorChooser.showDialog(frame, "Choose highlight color",
						highlightColor);
				if (color != null) {
		...
		private void filterPixels(DirectGif89Frame dgf) throws IOException {
			...
			for (int i = 0; i < npixels; ++i) {
				...
				if (colorIndex == -1) { // if it isn't in there yet
					...
					ciPixels[i] = (byte) colorIndex; // just send filtered
														// pixel
				}
			}
		}
	...
	abstract class Gif89Frame {
		...
		protected int theWidth = -1;
		protected int theHeight = -1;
		byte[] ciPixels;
		...
		void encode(OutputStream os, boolean epluribus, int color_depth, int transparent_index)
			...
			new GifPixelsEncoder(theWidth, theHeight, ciPixels, isInterlaced, color_depth)
					.encode(os);
		}
	...
	class GifPixelsEncoder {
		...
		// ----------------------------------------------------------------------------
		public GifPixelsEncoder(int width, int height, byte[] pixels, boolean interlaced,
				int colorDepth) {
			imgW = width;
			imgH = height;
			pixAry = pixels;
